...
QJsonObject str2json(QString str) {
    QJsonDocument jsonDoc = QJsonDocument::fromJson(str.toUtf8().data());
    if (jsonDoc.isNull()) {
        qDebug() << "read json obj from str failed: str = " << str.toLocal8Bit().data();
    }
...
QList<OnlineMessage *> DatabaseOperation::findAllMessages() {
    ...
    QSqlQuery query;
    if (query.exec("SELECT SessionID, MessageID, senderUsername, MessageText, Profile FROM Message")) {
        while (query.next()) {
            int sessionID = query.value(0).toInt();
            ...
            ret.append(msg);
            ServerDataCenter::Singleton().registerMessage(msg);
        }
    }
    return ret;
}
...
bool DatabaseOperation::insertUser(QString username, QString nickname, QString password, QJsonObject profile){
    ...
    ServerDataCenter::Singleton().registerUser(new OnlineUserModel(username, nickname, profile));
    return true;
}
...
int DatabaseOperation::insertSessionBasicInfo(QString SessionType, QJsonObject profile) {
    ...
    QString sessionName = profile.contains("SessionName") ? profile["SessionName"].toString() : "None";
    OnlineSession * session = new OnlineSession(sessionId, sessionName, profile, QList<QString>());
    auto & dcenter = ServerDataCenter::Singleton();
    dcenter.registerSession(session);
    return sessionId;
}
...
bool DatabaseOperation::insertMember(int sessionID, QString user){
    ...
    auto & dcenter = ServerDataCenter::Singleton();
    dcenter.getSession(sessionID).addMember(&dcenter.getUser(user));
    return true;
}

bool DatabaseOperation::insertMember(int sessionID, const OnlineUserModel &user){
    return insertMember(sessionID, user.getUsername().toUtf8().data());
}
...
//查询会话的所有历史消息
QList<int> DatabaseOperation::queryMessageBySession(int sessionID) {
    QList<int> ret;
    QSqlQuery query;
    QString sql = "SELECT MessageID FROM Message WHERE SessionID = (?)";
    query.prepare(sql); query.addBindValue(sessionID);
    if (!query.exec()) {
        qDebug() << "queryMessageBySession: " << query.lastError();
        return ret;
    }
    while (query.next()) {
...
OnlineUserModel & ServerDataCenter::getUser(QString username) {
    if (_getUser(username) == nullptr) throw "Not exist";
    return *users[username];
}

OnlineMessage & ServerDataCenter::getMessage(int SessionId, int MessageId) {
    if (_getMessage(SessionId, MessageId) == nullptr) throw "Not exist";
    return *messages[{SessionId, MessageId}];
}

OnlineSession & ServerDataCenter::getSession(int SessionId) {
    if (_getSession(SessionId) == nullptr) throw "Not exist";
    return *sessions[SessionId];
}

OnlineUserModel* ServerDataCenter::_getUser(QString username) {
    if (users.contains(username))
        return users[username];
    return nullptr;
}

OnlineSession* ServerDataCenter::_getSession(int SessionId) {
    if (sessions.contains(SessionId))
        return sessions[SessionId];
    return nullptr;
}

OnlineMessage* ServerDataCenter::_getMessage(int SessionId, int MessageId) {
    OnlineMessage * ret = nullptr;
    if (messages.contains(QPair<int,int>({SessionId, MessageId})))
        ret = messages[QPair<int,int>({SessionId, MessageId})];
    return ret;
}
...
OnlineUserModel * DatabaseOperation::findUser(QString username) {
    ...
    ServerDataCenter::Singleton().registerUser(ret);
    return ret;
}
...
OnlineSession * DatabaseOperation::findSession(int sessionID) {
    ...
    ServerDataCenter::Singleton().registerSession(ret);
    return ret;
}
...
OnlineMessage * DatabaseOperation::findMessage(int sessionId, int MessageId) {
    ...
    ServerDataCenter::Singleton().registerMessage(ret);
    return ret;
}

ServerDataCenter::ServerDataCenter(QObject *parent) : QObject(parent)
{
    connect(&DatabaseOperation::Singleton(), &DatabaseOperation::signal_DBstop, this, &ServerDataCenter::clean);
}

QList<QString> DatabaseOperation::findFriend(QString name){
    QSqlQuery query;
    QList<QString> result;
    QString sql = "SELECT Username FROM User WHERE Username LIKE '%"+name+"%' or Nickname LIKE '%"+name+"%'";
    query.prepare(sql);
    if (!query.exec()){
        qDebug() << query.lastError();
    }
