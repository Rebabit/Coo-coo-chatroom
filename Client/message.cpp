...
Message::Message(QString messagename,QWidget *parent): QWidget(parent)
{
    ...
    m_messageType = MessageType::textMessage;
    qDebug() <<"message type:text";
}

Message::Message(QString messagename,QString url,QWidget *parent): QWidget(parent)
{
    QFont textFont("MicrosoftYaHei", 12);
    this->setFont(textFont);
    MessageName = messagename;
    fileURL = url;
    m_messageType = MessageType::photo;
    qDebug() <<"message type:photo" ;
}
...
void Message::setTextContent(QString text, QString time, QSize size, UserType type, QString url)
{
    m_msg = text;
    m_userType = type;
    m_time = time;
    ...
    if(length > 10 ){
        ...
        m_meRightIcon = fitpixmap;
        m_otherLeftIcon = fitpixmap;
    }else{
        qDebug() << "111111111111";
...
void Message::paintEvent(QPaintEvent *event)
{
    ...
    if (m_messageType == MessageType::textMessage){
        if(m_userType == UserType::userOther) { // 聊天对象
            ...
            //内容
            QPen penText;
            penText.setColor(QColor(93,199,186));
    ...
    else if (m_messageType == MessageType::photo)
    {
        if(m_userType == UserType::userOther) { // 聊天对象
            qDebug() << "=========Userother_photo=========\n";
            //放置头像
            painter.drawPixmap(m_iconLeftRect, m_otherLeftIcon);
            QPixmap image1;
            QDir dir;
            ...
            int iconSpaceWidth = 20;      //头像与聊天界面框 长度距离
            int iconSpaceHeight= 10;      //头像与聊天界面框 高度距离
            painter.drawPixmap(iconSpaceWidth + iconWidth+30,iconSpaceHeight-10,70,70,image1);
        }
        else if(m_userType == UserType::userMe)
        {
            //头像
            painter.drawPixmap(m_iconRightRect, m_meRightIcon);
            QPixmap image1;
            ...
            int iconSpaceWidth = 20;      //头像与聊天界面框 长度距离
            int iconSpaceHeight= 10;      //头像与聊天界面框 高度距离
            painter.drawPixmap(this->width()-iconSpaceWidth - iconWidth-100,iconSpaceHeight-10,70,70,image1);
        }
        else if(m_userType == UserType::userTime) { // 时间
                   QPen penText;
                   penText.setColor(QColor(153,153,153));
